...
test("Nav Buttons Don't Work (Desktop & Mobile)", async () => {
  ...
  await setTimeout(10);
  assert.is(
    dom.window.document.getElementById('nav-logo').getAttribute('href'),
    '#header'
  );
  assert.is(
    dom.window.document.getElementById('nav-challenges').getAttribute('href'),
    '#challenges'
  );
  assert.is(
    dom.window.document.getElementById('nav-signup').getAttribute('href'),
    '#signup'
  );
  assert.is(
    dom.window.document
      .getElementById('mobile-nav-challenges')
      .getAttribute('href'),
    '#challenges'
  );
  assert.is(
    dom.window.document
      .getElementById('mobile-nav-signup')
      .getAttribute('href'),
    '#signup'
  );
});
...
test('Invert Banner Image Colors', async () => {
  ...
  assert.ok(
    ['invert(1)', 'invert(100%)'].includes(
      dom.window.getComputedStyle(banner)._values['filter']
    )
  );
});
  ...
  assert.ok(
    ['repeat(2,1fr)', '1fr1fr', '50%50%'].includes(
      dom.window
        ...
        ._values['grid-template-columns'].replace(/\s/g, '')
    )
  );
});
...
test('Improve Errors on Signup Form Validation (Empty Email)', async () => {
  ...
  assert.is(
    dom.window.document.getElementById('empty-error-message').hidden,
    false
  );
  assert.is(
    dom.window.document.getElementById('taken-error-message').hidden,
    true
  );
});
